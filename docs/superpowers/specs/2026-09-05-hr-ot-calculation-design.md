...
| File | Contents | Size |
|---|---|---|
| `CHECKTIME_InOutReportAll.xlsx` | Scanner punches: EMP ID, Name, Department, Date, Check In/Out 1-10, Total Hours, Location | 2,099 scan rows, 55 workers, Oct 2025-Aug 2026 |
...
2. **Roughly half the scan rows have a single punch** (1,001 of 2,099). The scanner
   was only being trialled during this period.
3. **The last row is not data.** CheckTime writes "This Report Generated by
   'Check-Time Attendance Management Software'" into the EMP ID column beneath the
   data. Counting it inflates the row count to 2,100 and the worker count to 56.
4. **Headcount does not reconcile**: 75 enrolled, 55 ever scanned, 83 in Million,
   85 on the payroll sheet.
5. **Employee codes do not match today.** Scanner uses `0018`, `2028`; Million uses
...
2. **Reader test.** Run the real `CHECKTIME_InOutReportAll.xlsx` through
   `checktime-reader` and confirm 2,099 scan rows and 55 workers, with the footer
   line dropped and the known single-punch rows flagged.
